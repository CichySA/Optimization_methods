...
A common stopping criterion was defined using the \textbf{maximum number of objective function evaluations}. Let \(B\) denote the computational budget assigned to a single run on a given instance. For the evolutionary algorithm, this budget is derived from its main control parameters as
\[
B = \text{pop\_size} \times \text{gen},
\]
with the initial population evaluations for EA not included for simplicity, counted as an algorithm set-up cost instead. The same value of \(B\) is then imposed on the remaining methods, so that all are compared under an equal limit on the number of candidate solutions evaluated.

An exception is made for the greedy algorithm, which always returns exactly one solution.
...
\[
\text{seed}_i = \text{base\_seed} \oplus (i \cdot K), \quad \forall, i \in {0,1,\dots,n-1}, ; K \in 2\mathbb{Z}+1
\]

where \(i\) is the run index, \(K\) is a large odd integer constant, and \(\oplus\) denotes the bitwise XOR operation.

This approach has several advantages. First, it guarantees that the same experimental setup always produces identical results. Second, by combining a base seed with an index, it avoids using consecutive seeds. The use of a large odd multiplier combined with the XOR operation reduces the risk of output correlations. This balances reproducibility and independence of runs.
...
% TODO: TFT calculation details and any optimization/caching.
